...
<div class="mb-2" style="font-size:0.7rem;color:#64748b;">
    <i class="bi bi-shield-check me-1"></i>Registro global de accesos a archivos de clientes — retencion 2 anos (RGPD Art. 30)
</div>
...
<div class="card mb-3">
    <div class="card-body py-2">
        <form method="GET" class="d-flex flex-wrap gap-2 align-items-end">
            <div>
                <label class="form-label small text-muted mb-0" style="font-size:0.7rem;">Cuenta</label>
                <select name="account_id" class="form-select form-select-sm" style="width:180px;background:#0f172a;border-color:#334155;color:#e2e8f0;font-size:0.8rem;">
                    <option value="">Todas</option>
                    <?php foreach ($accounts as $a): ?>
                    <option value="<?= $a['account_id'] ?>" <?= ($f['account_id'] ?? '') == $a['account_id'] ? 'selected' : '' ?>><?= View::e($a['account_domain'] ?? 'ID '.$a['account_id']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
            ...
            <a href="/admin/file-audit-log" class="btn btn-sm btn-outline-light" style="font-size:0.8rem;">Reset</a>
        </form>
    </div>
</div>
        ...
        <table class="table table-hover table-sm mb-0" style="font-size:0.8rem;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="/accounts/<?= $log['account_id'] ?>/audit-log" class="text-info text-decoration-none" style="font-size:0.8rem;">
                            <?= View::e($log['account_domain'] ?? $log['account_username']) ?>
                        </a>
                    </td>
                    <td>
