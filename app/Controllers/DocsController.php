    ...
    private function settingGuides(): array
    {
        return [
            ...
            'dns' => [
                ...
                'what_is' => 'Centraliza el proveedor DNS que Caddy usara para validar certificados del dominio del panel mediante DNS-01. En cristiano: si el panel vive en panel.ejemplo.com y no quieres abrir 80/443, esta pantalla guarda el proveedor DNS y el token API para que Caddy cree el TXT _acme-challenge y Let\'s Encrypt pueda emitir el certificado. Es independiente del gestor completo de registros Cloudflare.',
                'quick_steps' => [
                    'Abrir Settings > DNS.',
                    'Mirar el estado superior: TLS actual del panel, proveedor guardado y modulos DNS detectados en Caddy.',
                    'Elegir proveedor DNS del catalogo: Cloudflare, DigitalOcean, Route53, Hetzner, OVH, Vultr, Linode, Porkbun, Namecheap, Gandi, PowerDNS o RFC2136/BIND.',
                    'Si el proveedor aparece como no instalado, usar el bloque "Instalar modulo DNS en Caddy", introducir password admin y esperar a que Caddy quede recompilado.',
                    'Pegar el JSON de credenciales API del proveedor. El formulario muestra los campos esperados para cada proveedor.',
                    'Marcar "Activar DNS-01 como TLS del panel" si quieres que ese proveedor se aplique al certificado del dominio del panel al guardar.',
                    'Guardar DNS y comprobar que el certificado del panel pasa a ser publico cuando Caddy termine la emision.',
                ],
                'checklist' => [
                    'El dominio del panel esta configurado en Settings > Servidor, por ejemplo panel.ejemplo.com.',
                    'El email ACME esta configurado y es valido.',
                    'Caddy lista dns.providers.<proveedor> en la parte superior o en caddy list-modules.',
                    'Las credenciales API tienen permisos DNS suficientes para crear y borrar TXT _acme-challenge.',
                    'El proveedor elegido es el que gestiona realmente la zona DNS publica del dominio.',
                    'El dominio resuelve correctamente y no depende de la IP privada del servidor.',
                ],
                'pitfalls' => [
                    ...
                    'Pegar credenciales de otro proveedor no sirve aunque el dominio apunte bien por A/CNAME.',
                    'Activar DNS-01 sin dominio del panel o sin email ACME deja el proveedor guardado, pero no aplica el certificado.',
                    'Cloudflare DNS Manager sigue siendo otra pantalla: sirve para gestionar zonas/registros Cloudflare, no para todos los proveedores.',
                ],
                'advanced_steps' => [
                    'Flujo normal para panel cerrado: configurar dominio en Settings > Servidor, configurar proveedor en Settings > DNS, activar DNS-01 y guardar.',
                    'Flujo con Cloudflare heredado: si /etc/default/caddy ya tiene CLOUDFLARE_API_TOKEN, Cloudflare puede funcionar dejando el JSON vacio; para otros proveedores se exige JSON.',
                    'Flujo con proveedor nuevo: instalar modulo, confirmar que aparece como instalado, guardar credenciales y despues activar DNS-01.',
                    'Si falla la emision, revisar logs de Caddy buscando acme, challenge, dns.providers.<proveedor> y el hostname del panel.',
                    'Si no quieres DNS-01, volver a Settings > Servidor y elegir HTTP-01/TLS-ALPN-01 con 80/443 accesibles o certificado interno para acceso privado/IP.',
                ],
                'verify_commands' => [
                    'caddy list-modules | grep "^dns.providers"',
                    'journalctl -u caddy --since "30 minutes ago" --no-pager | egrep -i "acme|challenge|dns|certificate|error"',
                    'echo | openssl s_client -connect panel.ejemplo.com:8444 -servername panel.ejemplo.com 2>/dev/null | openssl x509 -noout -issuer -subject -dates',
                ],
                'rollback_steps' => [
                    'Entrar en Settings > Servidor.',
                    'Cambiar TLS del panel a HTTP-01/TLS-ALPN-01 o certificado interno segun el caso.',
                    'Guardar y comprobar acceso por dominio o IP fallback.',
                    'Si el problema fue el modulo Caddy, usar el backup indicado por el instalador en /var/backups/musedock/caddy/ o dejar que el rollback automatico actue si el servicio no arranca.',
                ],
            ],
            'cloudflare-dns' => [
